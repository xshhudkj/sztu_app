...
class SmartCacheManager(
    ...
    private fun initializeCacheLimitMonitoring() {
        scope.launch {
            try {
                val limitBytes = me.wcy.music.storage.preference.ConfigPreferences.getCacheLimitBytes()
                if (limitBytes > 0) {
    ...
    fun startAutoCleanService(intervalHours: Int) {
        try {
            me.wcy.music.service.AutoCacheCleanService.startService(context, intervalHours)
    ...
    fun stopAutoCleanService() {
        try {
            me.wcy.music.service.AutoCacheCleanService.stopService(context)
